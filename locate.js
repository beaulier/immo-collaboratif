...
{
	adresseActuel_ap = tab_adresse_ap[i];
...
function getCoords_ap(err, data)
{
	...
	zonename.bindPopup('<b>'+tab_intitule_ap[n]+'</b><br><p><a onclick="open_page_ap('+n+')" >Collaborez !</a></p>').openPopup();
	n++;
}
...
{
	var page = window.open("profil.php?identifiant="+identifiant+"", "Profil","menubar=no, status=no, scrollbars=no, menubar=no, width=800, height=600");
}

function open_page_ap(identifiant)
{
	var page = window.open("appart.php?identifiant="+identifiant+"", "Profil","menubar=no, status=no, scrollbars=no, menubar=no, width=1600, height=960");
}
