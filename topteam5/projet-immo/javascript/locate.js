...
var id = document.getElementById('identifiant').innerHTML;
var tab_id = state.split('#');
...
/*------------------------------------------------------------------------*/
/*Modification centre de la map demandée par l'utilisateur lors de l'input*/
/*------------------------------------------------------------------------*/
function search()
{
	...
	while (stop == false)
	{
		if (str != tab_name[i]){i++;}
		if(str == tab_name[i]){
			var name_ok = tab_name[i];
			stop = true;
		}
		else if (i == elements_length && str != tab_name[i]){
			alert("Votre ami est introuvable");
			stop = true;
		}
	}
	if (name_ok)
	{
		url = 'https://api.mapbox.com/geocoding/v5/mapbox.places/'+tab_adresse[i]+'.json?access_token=pk.eyJ1IjoiYmVhdWxpZXJ0aG9tYXMiLCJhIjoiY2loZGc1MXJoMGJzdHZxa2xub2ZqaHV1eSJ9.yYiNhz5x6szFz2r297zEsw';
		geocoder = L.mapbox.geocoder(url);
		coordinates = geocoder.query(tab_adresse[i], placeMap);
	}
	function placeMap(err, data)
	{
		lng = data.latlng[0];
		lat = data.latlng[1];
